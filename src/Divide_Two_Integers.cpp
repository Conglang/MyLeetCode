...
Well, division simply requires us to find how many times we can subtract the divisor from the the dividend without making the dividend negative.
...
//--------------------------------------------------------------------------------------------------------------
TEST_CASE("Divide_Two_Integers", "[Binary_Search]"){
	Solution s;
	SECTION("Normal Input"){
		REQUIRE(s.divide(-INT_MIN, -1) == INT_MAX);
		REQUIRE(s.divide(3, 0) == INT_MAX);
		REQUIRE(s.divide(15, 3) == 5);
		REQUIRE(s.divide(20, 7) == 2);
	}
}